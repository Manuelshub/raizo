...
staging-settings:
  user-workflow:
    workflow-name: "threat-detection"
  workflow-artifacts:
...
Example: For a workflow directory named `threat-detection` the command would be:

```bash
cd threat-detection && bun install
```
...
Example: For workflow named `threat-detection` the command would be:

```bash
cre workflow simulate ./threat-detection --target=staging-settings
```
