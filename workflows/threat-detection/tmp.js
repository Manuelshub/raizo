...
function buildSystemPrompt(confidenceThreshold = DEFAULT_CONFIDENCE_THRESHOLD) {
  return `
...
class HeuristicAnalyzer {
  ...
  score(telemetry) {
    const citations = [];
    let weightedSum = 0;
    const tvl1h = this.normalize(Math.abs(Math.min(0, telemetry.tvl.delta1h)), 0, 30);
    if (tvl1h > 0)
      citations.push("tvl.delta1h");
    weightedSum += tvl1h * this.weights.tvlDrop1h;
    const tvl24h = this.normalize(Math.abs(Math.min(0, telemetry.tvl.delta24h)), 0, 50);
    if (tvl24h > 0)
      citations.push("tvl.delta24h");
    weightedSum += tvl24h * this.weights.tvlDrop24h;
    const flash = this.normalize(telemetry.mempoolSignals.flashLoanBorrows, 0, 5);
    if (flash > 0)
      citations.push("mempoolSignals.flashLoanBorrows");
    weightedSum += flash * this.weights.flashLoanActivity;
    const failedTx = this.normalize(telemetry.transactionMetrics.failedTxRatio, 0, 0.3);
    if (failedTx > 0)
      citations.push("transactionMetrics.failedTxRatio");
    weightedSum += failedTx * this.weights.failedTxRatio;
    const priceDev = this.normalize(Math.abs(telemetry.priceData.priceDeviation), 0, 15);
    if (priceDev > 0)
      citations.push("priceData.priceDeviation");
    weightedSum += priceDev * this.weights.priceDeviation;
    const mempoolScore = this.normalize(telemetry.mempoolSignals.pendingLargeWithdrawals + telemetry.mempoolSignals.suspiciousCalldata.length, 0, 20);
    if (mempoolScore > 0)
      citations.push("mempoolSignals.pendingLargeWithdrawals");
    weightedSum += mempoolScore * this.weights.mempoolAnomalies;
    const threatScore = this.normalize(telemetry.threatIntel.activeCVEs.length * 3 + telemetry.threatIntel.darkWebMentions + Math.max(0, -telemetry.threatIntel.socialSentiment) * 5, 0, 20);
    if (threatScore > 0)
      citations.push("threatIntel.activeCVEs");
    weightedSum += threatScore * this.weights.threatIntelSignals;
    const stateRisk = (telemetry.contractState.pendingUpgrade ? 0.4 : 0) + this.normalize(telemetry.contractState.unusualApprovals, 0, 10) * 0.6;
    if (stateRisk > 0)
      citations.push("contractState.pendingUpgrade");
  ...
  normalize(value2, min, max) {
    if (max === min)
      return 0;
    return Math.max(0, Math.min(1, (value2 - min) / (max - min)));
  }
}
function mapSeverity(riskScore, threatCategory) {
  if (riskScore >= 0.95)
    return 3;
  if (riskScore >= 0.85)
    return 2;
...
function buildThreatReport(agentId, protocol, assessment, heuristicCitations) {
  const allCitations = [
  ...
  let reportId = "0x";
  for (let i2 = 0;i2 < reportIdInput.length; i2++) {
    reportId += reportIdInput.charCodeAt(i2).toString(16).padStart(2, "0");
  }
  return {
    reportId,
    agentId,
    ...
    confidenceScore: Math.floor(assessment.overallRiskScore * 1e4),
    evidenceHash: JSON.stringify(allCitations),
    timestamp: ts
  };
}
var HEURISTIC_GATE_THRESHOLD = 0.3;
function runSentinelPipeline(agentId, targetProtocol, telemetry, llmAssessment) {
  const heuristic = new HeuristicAnalyzer;
  const { baseRiskScore, citations: heuristicCitations } = heuristic.score(telemetry);
  ...
  if (llmAssessment.threatDetected && llmAssessment.overallRiskScore >= 0.5) {
    ...
    return buildThreatReport(agentId, targetProtocol, finalAssessment, heuristicCitations);
  }
  return null;
}
var sentinelConfigSchema = exports_external.object({
  schedule: exports_external.string().describe("Cron schedule for periodic sentinel sweeps"),
  chainSelector: exports_external.string().describe("EVM chain selector for EVMClient"),
  telemetryApiUrl: exports_external.string().describe("Endpoint serving TelemetryFrame JSON"),
  llmApiUrl: exports_external.string().describe("Endpoint for LLM risk analysis"),
  sentinelContractAddress: exports_external.string().describe("SentinelActions contract address"),
  agentId: exports_external.string().describe("Unique agent identifier"),
  targetProtocol: exports_external.string().describe("Protocol address to monitor"),
  confidenceThreshold: exports_external.number().min(0).max(1).default(0.85).describe("LLM confidence threshold — scores above this trigger protective action (spec §3.3)")
});
var onCronTrigger = (runtime2, _payload) => {
  const config = runtime2.config;
  runtime2.log("Sentinel sweep triggered");
  try {
    const httpCapability = new cre.capabilities.HTTPClient;
    const fetchTelemetry = (sendRequester, cfg) => {
    ...
    const telemetryBody = httpCapability.sendRequest(runtime2, fetchTelemetry, consensusIdenticalAggregation())(config).result();
    const telemetry = JSON.parse(telemetryBody);
    const heuristic = new HeuristicAnalyzer;
    const { baseRiskScore } = heuristic.score(telemetry);
    if (baseRiskScore < HEURISTIC_GATE_THRESHOLD) {
      runtime2.log(`Heuristic score ${baseRiskScore.toFixed(3)} below gate — skipping LLM`);
      return "skipped";
    }
    runtime2.log(`Heuristic score ${baseRiskScore.toFixed(3)} above gate — invoking LLM`);
    const fetchLLMAssessment = (nodeRuntime) => {
      const nodeHttpCapability = new cre.capabilities.HTTPClient;
      const nodeFetch = (sendRequester) => {
        const response = sendRequester.sendRequest({
          url: config.llmApiUrl,
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "x-goog-api-key": runtime2.getSecret({ id: "LLM_API_KEY", namespace: "raizo" }).result().value
          },
          body: JSON.stringify({
            system: buildSystemPrompt(config.confidenceThreshold),
            telemetry: JSON.parse(JSON.stringify(telemetry, (_, v) => typeof v === "bigint" ? v.toString() : v))
          })
        }).result();
        if (!ok(response)) {
      ...
      return nodeFetch(nodeRuntime);
    };
    const llmResultRaw = runtime2.runInNodeMode(fetchLLMAssessment, consensusIdenticalAggregation())().result();
    const assessment = JSON.parse(llmResultRaw);
    const report2 = runSentinelPipeline(config.agentId, config.targetProtocol, telemetry, assessment);
    if (!report2) {
      runtime2.log("No actionable threat — pipeline returned null");
      return "no_threat";
    }
    runtime2.log(`Threat detected: action=${report2.action} severity=${report2.severity} confidence=${report2.confidenceScore}`);
      ...
      args: [
        {
          ...
          agentId: report2.agentId,
          exists: true,
          targetProtocol: report2.targetProtocol,
          action: report2.action,
          severity: report2.severity,
          confidenceScore: report2.confidenceScore,
          evidenceHash: Buffer.from(report2.evidenceHash),
          timestamp: BigInt(report2.timestamp),
          donSignatures: "0x"
        }
      ]
    });
    const reportRequest = prepareReportRequest(writeData);
    runtime2.report(reportRequest).result();
    runtime2.log(`Report submitted on-chain: ${report2.reportId}`);
    return "reported";
  } catch (error) {
    runtime2.log(`Error in threat detection workflow: ${error}`);
    return "error";
  }
};
var initWorkflow = (config, _secretsProvider) => {
  const cron = new cre.capabilities.CronCapability;
  return [handler(cron.trigger({ schedule: config.schedule }), onCronTrigger)];
};
async function main() {
  const runner = await Runner.newRunner({
    configParser: (raw) => sentinelConfigSchema.parse(JSON.parse(new TextDecoder().decode(raw)))
  });
