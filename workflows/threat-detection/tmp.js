...
var fetchDexScreenerData = (nodeRuntime, protocolAddress) => {
...
var getGeminiAssessment = (nodeRuntime, frame, apiKey) => {
  const http = new ClientCapability2;
  const prompt = `
...
var analyzeProtocol = (nodeRuntime, protocol, apiKey) => {
...
var onCronTrigger = (runtime2) => {
  const { raizoCoreAddress, sentinelActionsAddress, chainName } = runtime2.config;
  ...
  runtime2.log("Raizo Workstream 9: Live Data Sentinel Started");
  let apiKey;
  try {
  ...
  const protocolsReply = evmClient.callContract(runtime2, {
    call: encodeCallMsg({
      from: zeroAddress,
      to: raizoCoreAddress,
  ...
  const protocols = decodeFunctionResult({
    abi: RAIZO_CORE_ABI,
    functionName: "getAllProtocols",
    data: bytesToHex(protocolsReply.data)
  });
  runtime2.log(`Successfully fetched ${protocols.length} protocol(s) from blockchain`);
  for (const protocol of protocols.slice(0, 2)) {
    if (!protocol.isActive)
      continue;
    const assessment = runtime2.runInNodeMode(analyzeProtocol, ConsensusAggregationByFields({
      overallRiskScore: median,
      threatDetected: identical,
      recommendedAction: identical,
      reasoning: identical
    }))(protocol, apiKey).result();
    if (assessment.recommendedAction !== "NONE" && assessment.threatDetected) {
      const reportData = {
        reportId: keccak256(stringToHex(protocol.protocolAddress + runtime2.now().getTime())),
        ...
        timestamp: BigInt(Math.floor(runtime2.now().getTime() / 1000)),
      ...
      runtime2.log(`Safeguard report submitted via Gemini: ${assessment.recommendedAction}`);
    }
  }
  return "Success";
};
