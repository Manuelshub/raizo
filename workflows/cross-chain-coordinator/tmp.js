...
function evaluateScope(event, deployment) {
  if (event.severity === 3) {
    return "ALL_CHAINS";
  }
...
function resolveTargetChains(event, scope, monitoredChains) {
  switch (scope) {
    case "ALL_CHAINS":
    case "SAME_PROTOCOL":
    case "RELATED_ALERT":
...
function buildPropagationMessages(event, scope, monitoredChains) {
...
function runCoordinatorPipeline(event, deployment, monitoredChains) {
  const scope = evaluateScope(event, deployment);
  return buildPropagationMessages(event, scope, monitoredChains);
}
function parseThreatReportedEvent(eventData) {
  return {
...
var coordinatorConfigSchema = exports_external.object({
  chainSelector: exports_external.string().describe("Source chain selector for EVMClient"),
  sentinelContractAddress: exports_external.string().describe("SentinelActions contract emitting ThreatReported events"),
  crossChainRelayAddress: exports_external.string().describe("CrossChainRelay contract address for CCIP messaging"),
  deploymentRegistryUrl: exports_external.string().describe("API endpoint returning ProtocolDeployment JSON"),
  monitoredChainSelectors: exports_external.array(exports_external.string()).describe("Chain selectors for all monitored chains"),
  monitoredChainIds: exports_external.array(exports_external.number()).describe("Numeric chain IDs for monitored chains")
});
var onThreatReported = (runtime2, eventPayload) => {
  const config = runtime2.config;
  runtime2.log("ThreatReported event received");
  try {
    const event = parseThreatReportedEvent(eventPayload.log);
    runtime2.log(`Processing report ${event.reportId} severity=${event.severity}`);
    const httpCapability = new cre.capabilities.HTTPClient;
    const fetchDeployment = (sendRequester, cfg) => {
    ...
    const deploymentBody = httpCapability.sendRequest(runtime2, fetchDeployment, consensusIdenticalAggregation())(config).result();
    const deployment = JSON.parse(deploymentBody);
    const messages = runCoordinatorPipeline(event, deployment, config.monitoredChainIds);
    if (messages.length === 0) {
      runtime2.log("Scope=LOCAL_ONLY — no cross-chain propagation needed");
      return "local_only";
    }
    runtime2.log(`Propagating to ${messages.length} chains`);
    for (const msg of messages) {
      const targetChainSelector = config.monitoredChainSelectors[config.monitoredChainIds.indexOf(msg.destChain)];
      if (!targetChainSelector) {
        runtime2.log(`No selector for chain ${msg.destChain} — skipping`);
        continue;
      }
      const destChainSelector = config.monitoredChainSelectors[config.monitoredChainIds.indexOf(msg.destChain)];
      if (!destChainSelector) {
        runtime2.log(`No chain selector for chain ${msg.destChain}, skipping`);
        continue;
      }
      ...
      runtime2.log(`Dispatched to chain ${msg.destChain}: ${msg.reportId}`);
    }
    return "propagated";
  } catch (error) {
    runtime2.log(`Error in coordinator workflow: ${error}`);
    return "error";
  }
};
var coordinatorInitWorkflow = (config) => {
