...
var runGeminiReportEnhancement = (nodeRuntime, baseReport, apiKey) => {
...
var generateReport = (nodeRuntime, apiKey) => {
  ...
  const enhancement = runGeminiReportEnhancement(nodeRuntime, baseReport, apiKey);
  return {
    ...baseReport,
...
var onCronTrigger = (runtime2) => {
  const { complianceVaultAddress, chainId, agentId, chainName } = runtime2.config;
  runtime2.log("=== Raizo Compliance Reporter: Starting AI-Enhanced Generation ===");
  ...
  if (!network248) {
    runtime2.log(`ERROR: Unknown chain name: ${chainName}`);
    throw new Error(`Unknown chain name: ${chainName}`);
  }
  runtime2.log(`Chain resolved: ${chainName} -> selector ${network248.chainSelector.selector}`);
  const evm = new ClientCapability(network248.chainSelector.selector);
  let apiKey;
  try {
    apiKey = runtime2.getSecret({ id: "API_KEY" }).result().value;
    runtime2.log("AI_API_KEY secret loaded successfully");
  } catch (e) {
    runtime2.log("Warning: AI_API_KEY secret not found, using simulation fallback");
    apiKey = "AI_API_KEY_SIMULATION_FALLBACK";
  }
  runtime2.log("Generating compliance report with AI enhancement...");
  const report2 = runtime2.runInNodeMode(generateReport, ConsensusAggregationByFields({
    reportId: identical,
  ...
  runtime2.log(`Report generated: ${report2.reportId}`);
  runtime2.log(`Summary: ${report2.summary}`);
  runtime2.log(`Recommendations: ${report2.recommendations.length} items`);
  const reportData = JSON.stringify(report2);
  const reportHash = keccak256(stringToHex(reportData));
  const reportURI = "ipfs://compliance-reports/" + report2.reportId;
  runtime2.log(`Anchoring Gemini-enhanced report: ${report2.reportId}`);
  runtime2.log(`Report hash: ${reportHash}`);
  evm.callContract(runtime2, {
    call: encodeCallMsg({
      from: runtime2.config.operatorAddress,
      ...
      data: encodeFunctionData({
        ...
        args: [
          reportHash,
          agentId,
          FRAMEWORK_MAP[report2.framework] || 5,
          chainId,
          reportURI
        ]
      })
  ...
  runtime2.log(`✅ AI-Enhanced Report anchored successfully`);
  return report2.reportId;
};
