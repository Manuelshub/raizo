...
function evaluateRule(rule, metrics, sanctionsList) {
  const value2 = metrics[rule.condition.metric];
  ...
  switch (rule.condition.operator) {
    ...
    case "eq":
      return value2 === threshold;
    case "in":
      return threshold.includes(value2);
    case "matches":
      return sanctionsList.includes(value2);
    default:
      return false;
  }
}
function generateComplianceReport(chainId, findings) {
  ...
  return {
    ...
    recommendations: score < 100 ? ["Review flagged transactions", "Update risk parameters"] : []
  };
}
function runCompliancePipeline(chainId, rules, metrics, sanctionsList) {
  const findings = [];
  const now = Math.floor(Date.now() / 1000);
...
var complianceConfigSchema = exports_external.object({
  schedule: exports_external.string().describe("Cron schedule for periodic compliance checks"),
  chainSelector: exports_external.string().describe("EVM chain selector"),
  chainId: exports_external.number().describe("Numeric chain ID for report metadata"),
  agentId: exports_external.string().describe("CRE agent identifier (bytes32 hex)"),
  metricsApiUrl: exports_external.string().describe("Endpoint serving protocol metrics JSON"),
  sanctionsApiUrl: exports_external.string().describe("Endpoint serving sanctions list"),
  rulesApiUrl: exports_external.string().describe("Endpoint serving RegulatoryRule[] JSON"),
  complianceVaultAddress: exports_external.string().describe("ComplianceVault contract address")
});
var onComplianceTrigger = (runtime2, _payload) => {
  const config = runtime2.config;
  runtime2.log("Compliance check triggered");
  try {
    const httpCapability = new cre.capabilities.HTTPClient;
    const fetchRules = (sendRequester, cfg) => {
    ...
    const rulesBody = httpCapability.sendRequest(runtime2, fetchRules, consensusIdenticalAggregation())(config).result();
    const rules = JSON.parse(rulesBody);
    const fetchMetrics = (sendRequester, cfg) => {
      const response = sendRequester.sendRequest({
    ...
    const metricsBody = httpCapability.sendRequest(runtime2, fetchMetrics, consensusIdenticalAggregation())(config).result();
    const metrics = JSON.parse(metricsBody);
    const fetchSanctions = (nodeRuntime) => {
    ...
    const sanctionsRaw = runtime2.runInNodeMode(fetchSanctions, consensusIdenticalAggregation())().result();
    const sanctionsList = JSON.parse(sanctionsRaw);
    const report2 = runCompliancePipeline(config.chainId, rules, metrics, sanctionsList);
    runtime2.log(`Compliance report generated: score=${report2.riskSummary.complianceScore} findings=${report2.findings.length}`);
    if (report2.findings.length > 0) {
      const writeData = encodeFunctionData({
        abi: complianceVaultAbi,
        functionName: "storeReport",
        args: [
          report2.metadata.reportId,
          config.agentId,
          1,
          config.chainId,
          `ipfs://raizo-report-${report2.metadata.reportId}`
        ]
      });
      runtime2.report(prepareReportRequest(writeData)).result();
      runtime2.log(`Report hash anchored on-chain: ${report2.metadata.reportId}`);
    }
    return report2.riskSummary.complianceScore === 100 ? "compliant" : "flagged";
  } catch (error) {
    runtime2.log(`Error in compliance workflow: ${error}`);
    return "error";
  }
};
var complianceInitWorkflow = (config) => {
