...
staging-settings:
  user-workflow:
    workflow-name: "compliance-reporter"
  workflow-artifacts:
...
Example: For a workflow directory named `compliance-reporter` the command would be:

```bash
cd compliance-reporter && bun install
```
...
Example: For workflow named `compliance-reporter` the command would be:

```bash
cre workflow simulate ./compliance-reporter --target=staging-settings
```
